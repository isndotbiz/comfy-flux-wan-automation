...
def generate_image(prompt, width=1024, height=1024):
    workflow = {
        "1": {"inputs": {"width": width, "height": height, "batch_size": 1}, "class_type": "EmptyLatentImage"},
        "2": {"inputs": {"text": prompt, "clip": ["11", 0]}, "class_type": "CLIPTextEncode"},
        "3": {"inputs": {"text": "blurry, low quality", "clip": ["11", 0]}, "class_type": "CLIPTextEncode"},
        "4": {"inputs": {"seed": int(time.time()), "steps": 20, "cfg": 7.0, "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0, "model": ["10", 0], "positive": ["2", 0], "negative": ["3", 0], "latent_image": ["1", 0]}, "class_type": "KSampler"},
        "5": {"inputs": {"samples": ["4", 0], "vae": ["12", 0]}, "class_type": "VAEDecode"},
        "6": {"inputs": {"filename_prefix": f"instagram_{int(time.time())}", "images": ["5", 0]}, "class_type": "SaveImage"},
        "10": {"inputs": {"unet_name": "flux1-dev.safetensors"}, "class_type": "UNETLoader"},
        "11": {"inputs": {"clip_name1": "t5xxl_fp16.safetensors", "clip_name2": "clip_l.safetensors"}, "class_type": "DualCLIPLoader"},
