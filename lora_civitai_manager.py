...
class CivitAIManager:
    ...
    def get_popular_instagram_loras(self) -> List[Dict]:
        ...
        sorted_loras = sorted(unique_loras.values(), 
    ...
    def download_lora(self, model_id: int, version_id: int = None) -> Optional[str]:
        ...
        try:
            ...
            if version_id:
                version = next((v for v in model_versions if v["id"] == version_id), model_versions[0])
            else:
...
class LoRAOptimizedGenerator:
    ...
    def generate_with_lora(self, prompt: str, lora_info: Dict, strength: float = 0.8) -> Optional[str]:
        ...
        print(f"📝 Trigger words: {', '.join(trigger_words) if trigger_words else 'None'}")
        
        workflow = self.create_lora_workflow(prompt, lora_filename, strength, trigger_words)
    ...
    if choice == "1":
        ...
        if loras:
            ...
            for i, lora in enumerate(loras[:10], 1):
                ...
                print(f"    ID: {lora['id']} | Rating: {rating:.1f} | Downloads: {downloads:,}")
        ...
        if query:
            ...
            for i, lora in enumerate(loras, 1):
                rating = lora.get("stats", {}).get("rating", 0)
                print(f"{i:2d}. {lora['name']} (ID: {lora['id']}) | Rating: {rating:.1f}")
        ...
        try:
            ...
            if 0 <= lora_idx < len(downloaded_loras):
                ...
                if prompt:
                    strength = float(input("LoRA strength (0.1-1.0, default 0.8): ").strip() or "0.8")
                    
                    prompt_id = generator.generate_with_lora(prompt, selected_lora, strength)
                    if prompt_id:
                        print("\n✅ Generation started!")
