#!/usr/bin/env python3
"""
Instagram Model Generator with LoRA Support - Fixed Version
Enhanced version with proper Flux model loading for WAN2.1
"""
import json
import time
import requests
...
class InstagramLoRAGenerator:
    ...
    def create_flux_lora_workflow(self, prompt, seed=None, width=1024, height=1024):
        """Create proper Flux workflow with LoRA support using WAN2.1 architecture"""
        if seed is None:
        ...
        workflow = {
            ...
            "2": {
                "inputs": {
                    "text": "blurry, low quality, distorted, amateur, bad anatomy, deformed, ugly, pixelated, watermark, text, signature, nsfw",
                    "clip": ["12", 1]
                },
                "class_type": "CLIPTextEncode"
            },
            ...
            "10": {
                "inputs": {
                    "filename_prefix": f"instagram_lora_{int(time.time())}",
                    "images": ["8", 0]
                },
...
def main():
    ...
    print("🎨 Instagram Model Generator with Flux.1 LoRA - Fixed Version")
    print("=" * 60)
