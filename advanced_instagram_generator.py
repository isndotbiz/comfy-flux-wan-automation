...
class OllamaOptimizer:
    ...
    def optimize_prompt(self, base_prompt: str, style: str = "instagram", lora_triggers: List[str] = None) -> Tuple[str, str]:
...
"""
    ...
    def _fallback_enhance(self, prompt: str, lora_triggers: List[str] = None) -> Tuple[str, str]:
        """Fallback prompt enhancement"""
        quality_words = [
            "high quality", "detailed", "professional", "photorealistic",
            "sharp focus", "beautiful lighting", "masterpiece", "8K resolution",
            "hyperdetailed", "award winning photography"
        ]
        
        instagram_style = [
            "instagram style", "social media ready", "aesthetic", "trendy",
            "modern photography", "lifestyle photo", "influencer style"
        ]
        ...
        selected_enhancements = random.sample(enhancement_words, min(5, len(enhancement_words)))
    ...
    def _generate_negative_prompt(self) -> str:
        ...
        return ("blurry, low quality, distorted, amateur, bad anatomy, deformed, ugly, pixelated, "
...
class AdvancedInstagramGenerator:
    def __init__(self):
        ...
        self.output_dir = Path("/workspace/comfy-flux-wan-automation/workflows/advanced")
    ...
    def get_suggested_loras(self, prompt: str) -> List[Dict]:
        ...
        for lora in self.loras:
            ...
            if any(keyword in prompt_lower for keyword in portrait_keywords):
                if any(tag in ["portrait", "realistic", "photography"] for tag in lora_tags):
                    lora_score += 3
                if "portrait" in lora_name or "realistic" in lora_name:
                    lora_score += 2
            
            if any(keyword in prompt_lower for keyword in realistic_keywords):
                if any(tag in ["realistic", "photography", "photo"] for tag in lora_tags):
                    lora_score += 2
    ...
    def generate_instagram_batch(
        ...
        print(f"🤖 Ollama optimization: {'✅' if self.optimizer.is_available() else '❌ (fallback)'}")
...
def main():
    ...
    print("=" * 50)
    print(f"🤖 Ollama status: {'✅ Connected' if generator.optimizer.is_available() else '❌ Offline (using fallback)'}")
    ...
    if choice == "1":
        ...
        if prompt:
            results = generator.generate_instagram_batch([prompt], use_lora=True, auto_select_lora=True)
            if results:
        ...
        results = generator.generate_instagram_batch(base_prompts, use_lora=True, auto_select_lora=True)
